...
impl ReliableSender {
    ...
    /// 以"可靠"的方式发送消息给指定地址。
    /// "可靠"意味着：如果一次发送不成功，后台会持续不断地重试，直到成功收到 ACK 或者被取消。
    ///
    /// 参数:
    /// - address: 目标节点的 Socket 地址。
    /// - data: 要发送的二进制数据。
    ///
    /// 返回值:
    /// - CancelHandler: 一个 oneshot::Receiver。调用者可以通过 drop 它来取消发送；或者通过 await 它来等待发送成功收到的 ACK 数据。
    pub async fn send(&mut self, address: SocketAddr, data: Bytes) -> CancelHandler {
        // 1. 创建一个 oneshot 通道，作为"取消开关"和"回执单"。
        // - sender: 我们把它传给后台 Connection 任务。后台任务如果发送成功，会通过它是通知我们；或者通过检测它被 drop 来感知我们想取消任务。
        // - receiver (即 CancelHandler): 我们把这个手柄返回给调用者。
        let (sender, receiver) = oneshot::channel();

        // 2. 查找或创建与目标地址的连接任务。
        // - self.connections 是一个 HashMap，缓存了所有活跃的连接。
        // - entry(address).or_insert_with(...): 如果还没和这个地址建立过连接，就调用 spawn_connection 启动一个新的后台 TCP 维护任务。
        // - .send(...): 将要发送的数据和控制句柄(sender)一起打包扔进那个后台任务的邮箱(Channel)。
        self.connections
            .entry(address)
        ...
        // 3. 返回控制句柄给调用者。
        // 注意：这里返回时，数据可能还在缓冲区排队，网络包还没发出去。真正的发送由后台任务异步完成。
        receiver
    }
